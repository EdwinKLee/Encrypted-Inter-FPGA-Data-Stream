...
(
	...
	while (true)
	{
		...
		if (src == 4) {
			...
			printf("\nprint print print: %x\n", read_data[0]);
			std::cout<<std::endl<< "ACTUAL PACKET " << std::hex << read_data[0] << std::dec << std::endl;
			std::cout<<std::endl<< "ACTUAL PACKET " << std::hex << ret_text << std::dec << std::endl;
...
#pragma HLS unroll
       ...
       //Lp13:for (i = 0; i < AES_BLOCK_SIZE - 1; ++i) {
//#pragma HLS unroll
          // cipherT |= cipher[i];
          // cipherT <<= 8;
       //}
...
#pragma HLS unroll
   ...
   /*******************************************************************/
   // else if(gp.id == 5) {
    //    IN cipherT;// = (gp.data)(127,0);

       //
       // store the cipher into on-chip mems
       // IN cipherT = in.read();
    //    lp1:for (i = AES_BLOCK_SIZE; i > 0 ; --i) {
...
//        plainT |= plain[15];
       ...
       // gp.dest = 4;
       // gp.last = 1;
       // (gp.data)(127,0) = plainT;
       // out.write(gp);

       out->packet_write((char *)plain, 1, 0, 4);

    //    IO_DATA_TYPE golden_plain ("0123456789abcdeffedcba9876543210", 16);

    //    if (plainT == golden_plain) {
